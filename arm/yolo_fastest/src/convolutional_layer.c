...
//    else
//    {
//    	b = malloc(out_w*out_h*para.input_c*para.kernel_size*para.kernel_size*sizeof(float));
...
void convolutional_compute(layer_params para, float* input, float* weight, float* output)
{
    ...
    int groups = (para.groups > 0) ? para.groups : 1;
    int c_per_group = para.input_c / groups;
    int n_per_group = para.kernel_n / groups;
    int k_group = para.kernel_size * para.kernel_size * c_per_group;
    int n = out_w * out_h;

    int g;
    for (g = 0; g < groups; ++g) {
        float *im = input + (size_t)g * c_per_group * para.input_h * para.input_w;
        float *a = weight + (size_t)g * c_per_group * n_per_group * para.kernel_size * para.kernel_size;
        float *c = output + (size_t)g * n_per_group * out_h * out_w;
        float *b;
        if (para.kernel_size == 1) {
            b = im;
        } else {
            b = malloc((size_t)out_w * out_h * c_per_group * para.kernel_size * para.kernel_size * sizeof(float));
            my_im2col_cpu(im, c_per_group, para.input_h, para.input_w, para.kernel_size, para.stride, para.pad, b);
        }
        my_gemm(0, 0, n_per_group, n, k_group, 1.f, a, k_group, b, n, 1.f, c, n);
        if (para.kernel_size != 1)
            free(b);
    }
}
