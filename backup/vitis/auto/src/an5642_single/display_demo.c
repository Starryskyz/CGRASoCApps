...
void CGRA_Func(u32* src, u8* gray, u32* dst) {
...
//	vdma_write_init(&camera_vdma, XPAR_AXIVDMA_0_DEVICE_ID,1280 * 4,720,1280 * 4,(unsigned int)pFrames[0]);
	...
	////////////////////////////
//	Status = XScuGic_Connect(&RunCfg.IntrPtr, KEY_INTR_ID, (Xil_InterruptHandler)GpioHandler, &GpioInstance);
//	if (Status != XST_SUCCESS) {
//		xil_printf("XScuGic_Connect failure for GPIO interrupt ID %d\r\n", KEY_INTR_ID);
//		return XST_FAILURE;
//	}
//
//	XScuGic_SetPriorityTriggerType(&RunCfg.IntrPtr, KEY_INTR_ID, 0x00, 0x3);
//
//	XScuGic_Enable(&RunCfg.IntrPtr, KEY_INTR_ID);
	///////////////////////////////////
...
//			CGRA_flag = 1;
			if(CGRA_en){
				xil_printf("CGRA EX\r\n");
				CGRA_Func(pFrames[0], gray, pFrames[0]);
				//CGRA_ex();
				CGRA_en = 0;
			}
		}else{
